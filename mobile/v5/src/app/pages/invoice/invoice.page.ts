...
export class InvoicePage extends Service<Token> {
  ...
  updateQuantity(id, quantity, itemid, itemno, description) {
    ...
    if(id) {
      me.commonSQL.updateDataQuery("UPDATE tblContractExport SET dblQuantity = (dblQuantity - " + (quantity || 0) + ") WHERE intContractDetailId = \'" + id + '\'').then(res => console.log('UPDATED CONTRACTS'));
    }
  }
  ...
  async applyContract(order) {
    ...
    me.commonSQL.getDataQuery('SELECT * FROM tblContractExport WHERE strEntityNo = \'' + me.customerNumber + '\'').then(res => {
      ...
      if(me.contract.length > 1) {
        loader.dismiss();
        me.openCustomerContract();
      }
      else if(me.contract.length == 1) {
        loader.dismiss();
  ...
  async applyCustomerContract(contracts) {
    var me = this;
    ...
    contracts.forEach(contract => {
      ...
      _item.dblPrice = contract['dblCashPrice'];
      ...
      _item.ysnSelected = false;
      _item['price'] = contract['dblCashPrice'];
      _item['quantity'] = 0;
      _item['totalamount'] = 0;
      _item['InvoiceTaxCodes'] = [];
      _item['tblOrderTaxCode'] = [];
  ...
  async applyMultiLevelPricing(order) {
    ...
    me.commonSQL.getDataQuery('SELECT * FROM tblCustomerMultiLevelPricing where strPriceLevel = \'' + me.customer['strLevel'] + '\' and strItemNo = \'' + order.strItemNo + '\' and intEntityId = \'' + me.customer['intEntityId'] + '\' and intCompanyLocationPricingLevelId = \'' + me.globalInfo.intLocation + '\' LIMIT 1').then(res => {
      me.pricing = JSON.parse(JSON.stringify(res));
      console.log(me.pricing);
  ...
  buildInvoiceItemsReport() {
    ...
    for(var x = 0; x <= me.invoice['length'] - 1; x++) {
      ...
      for(var y = 0; y <= record['tblOrderItem']['length'] - 1; y++) {
        ...
        _body.table.body.push([{text: item.strItemNo + ' - ' + item.strItemDescription, alignment: 'left'}, {text: parseInt(item.quantity).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') || 0.00, alignment: 'right'}, {text: (item.ending || '0') + '%', alignment: 'right'}, {text: item.strContractNumber || '', alignment: 'right'},{text: item.dblAvailableQty || '', alignment: 'right'},{text: (item.dblPrice || 0.00).toString(), alignment: 'right'},{text: parseFloat(item['totalamount'] || 0).toFixed(2).toString().match(/^-?\d+(?:\.\d{0,2})?/)[0] || 0.00, alignment: 'right'}]);
        ...
        // HEADER
        if(me.isMeter == true && (item.dblMeterTotalizer || 0) > 1) {
          ...
          _header_item.table.body.push(['', '',{ text: 'Start: 0.0  End: ' + (item.dblQuantity || item.quantity), alignment: 'left' }]);
