...
export class CustomerInvoiceAddSitePage {
  ...
  ionViewDidLoad() {
    ...
    me.consumptionsiteBRL.getByQuery('select consumptionsite.intSiteNumber, consumptionsite.strDescription, device.strSerialNumber, device.dblTankCapacity from consumptionsite left join device on device.intSiteId = consumptionsite.intSiteId where consumptionsite.strEntityNo = \'' + me.currentCustomer + '\' order by consumptionsite.intSiteNumber').then(res => {
  ...
  loadItemQuery() {
    ...
    me.consumptionsiteBRL.getByQuery('select consumptionsite.intSiteNumber, consumptionsite.strDescription, device.strSerialNumber, device.dblTankCapacity from consumptionsite left join device on device.intSiteId = consumptionsite.intSiteId where consumptionsite.strEntityNo = \'' + me.currentCustomer + '\' and (consumptionsite.intSiteNumber like \'%' + me.searchValue + '%\' or consumptionsite.strDescription like \'%' + me.searchValue + '%\' or device.strSerialNumber like \'%' + me.searchValue + '%\' or device.dblTankCapacity like \'%' + me.searchValue + '%\') order by intSiteNumber').then(res => {
      me.data = JSON.parse(JSON.stringify(res));
      console.log(me.data);
