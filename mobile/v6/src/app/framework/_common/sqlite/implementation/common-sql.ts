...
import { SMPaymentMethod } from '../../../models/tblSMPaymentMethod';
...
export class CommonSQL implements ICommonSQL {
  ...
  importDataFromApp(sqlFile: any): Promise<any> {
    var me = this;
    return new Promise((resolve, reject) => {
  ...
  DropAndCreate(table, createAll): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (tablename == 'tblSMPaymentMethod' || createAll == true) {
        me.dropTable(new SMPaymentMethod()).then(res => {
          console.log(res);
          me.counter++;
          me.createTable(new SMPaymentMethod()).then(res => {
            console.log(res);
            me.counter++;
            console.log(tablename);
            if (createAll == false) { resolve(table); }
