...
@IonicPage()
@Component({
  selector: 'page-customer-invoice-add-item',
  templateUrl: 'customer-invoice-add-item.html',
})
export class CustomerInvoiceAddItemPage {
  data: any;
  currentSelected: any;
  ...
  ionViewDidLoad() {
    ...
    // me.itemBRL.getByQuery('select distinct strItemNo,strDescription,dblSalePrice,intItemId from item where intItemLocationId = \'' + me.locationId + '\' order by strItemNo').then(res => {
  ...
  loadItemQuery() {
    ...
    var _query = 'select distinct strItemNo,strDescription,dblSalePrice,intItemId from item where intItemLocationId = \'' + me.locationId + '\' order by strItemNo';
    if(me.searchValue) {
      _query = 'select distinct strItemNo,strDescription,dblSalePrice,intItemId from item where intItemLocationId = \'' + me.locationId + '\' and (strItemNo like \'%' + me.searchValue + '%\' or strDescription like \'%' + me.searchValue + '%\' or dblSalePrice like \'%' + me.searchValue + '%\') order by strItemNo';
    }
