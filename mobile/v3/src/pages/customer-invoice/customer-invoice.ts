...
export class CustomerInvoicePage extends BaseService<DeviceToken> {
  ...
  constructor(public navCtrl: NavController, 
    ...
    this.isFromCustomer = navParams.get('isFromCustomer') || false;
    console.log(this.invoice);

    if(!Array.isArray(this.invoice) && this.invoice) {
      var _customernumber = this.invoice;
      this.invoice = [];
      this.currentOrder['strCustomerNumber'] = _customernumber;
    }
    else if(this.customer && !this.invoice) {
      this.invoice = [];
      this.currentOrder = this.customer;
    }
  ...
  getCustomer() {
    ...
    var _customernumber = me.currentOrder['strCustomerNo']  || me.currentOrder['strCustomerNumber'] || me.invoice[0].strCustomerNo;
          ...
          for(var x = 0; x <= me.orders.length-1; x++) {
            for(var y = 0; y <= me.items.length-1; y++) {
              if(me.orders[x].intOrderId == me.items[y].intOrderId) {
                if(_previousSiteId != me.orders[x].intSiteId) {
                  _previousX = x;
                }
                if(me.itemtaxcode.length > 0) {
                  ...
                  me.orders[_previousX]['tblOrderItem'] = [];
                  me.orders[_previousX]['tblOrderItem'].push(me.items[y]);
                }
                else {
                  me.orders[_previousX]['tblOrderItem'] = [];
                  me.orders[_previousX]['tblOrderItem'].push(me.items[y]);
            ...
            _previousSiteId = me.orders[x].intSiteId;            
          }
  ...
  openAddItem() {
    ...
    me.events.subscribe('addInvoiceItem', (paramsVar) => {
      ...
      me.events.unsubscribe('addInvoiceItem');
    });

    me.navCtrl.push("CustomerInvoiceAddItemPage", { intLocation: me.globalInfo['intLocation'] });
  }
  
  openAddSite() {
    var me = this;
    me.events.subscribe('addInvoiceSite', (paramsVar) => {
      console.log(paramsVar);
      ...
      me.events.unsubscribe('addInvoiceSite');
    });

    me.navCtrl.push("CustomerInvoiceAddSitePage", { customerid: me.currentOrder['strCustomerNo'] || me.currentOrder['strCustomerNumber'] || me.customer['strCustomerNumber'] });
  }
  ...
  openAddTaxCode() {
    ...
    me.events.subscribe('addInvoiceTaxCode', (paramsVar) => {
      ...
      if(me.selectedItems.length > 0) {
        ...
        // if(me.selectedItems[_lastItem]["tblOrderTaxCode"].length > 0) {
        //   me.selectedItems[_lastItem]["tblOrderTaxCode"].push(_taxcode);
        // }
        // else {
        //   me.selectedItems[_lastItem]["tblOrderTaxCode"] = [];
        //   me.selectedItems[_lastItem]["tblOrderTaxCode"].push(_taxcode);
        // }        
      }
      ...
      //   // _taxcode.dblTotalTax = (_taxcode.dblRate || 0) * (_quantity || 0);
      //   me.currentOrder["tblOrderItem"][_lastItem]["tblOrderTaxCode"][me.currentOrder["tblOrderItem"][_lastItem]["tblOrderTaxCode"].length || 0] = _taxcode;
      // }    
  ...
  moreDetail() {
    ...
    this.navCtrl.push("CustomerDetailPage", { customerid: this.currentOrder['strCustomerNo'] || this.invoice[0].strCustomerNo || this.currentOrder['strCustomerNumber'] });
  }
  ...
  processInvoice() {
    ...
    me.invoiceBRL.add(_invoice).then(res => {
      ...
      for(var x = 0; x <= me.currentOrder["tblOrderItem"].length - 1; x++) {
        ...
        _invoiceItems.dblPrice = me.currentOrder['tblOrderItem'][x].dblPrice || 0;
  ...
  buildInvoiceItemsReport() {
    ...
    for(var x = 0; x <= me.invoice['length'] - 1; x++) {
      ...
      for(var y = 0; y <= record['tblOrderItem']['length'] - 1; y++) {      
        ...
        if(!item.dblPrice) item.dblPrice = 0;
            ...
            for(var x = 0; x<=_invoice.length-1; x++) {
              ...
              for(var y = 0; y<=_invoiceitems.length-1; y++) {
                ...
                if(_invoice[x].intOrderId == _invoiceitems[y].intOrderId) {
                  if(!_temp_invoice['tblMBILInvoiceItems']) _temp_invoice['tblMBILInvoiceItems'] = [];
                  ...
                  for(var z = 0; z<=_invoicetaxcodes.length-1; z++) {                      
                    ...
                    if(_invoice[x].intOrderId == _invoicetaxcodes[z].intOrderId) {                                                
                      if(!_temp_invoice['tblMBILInvoiceItems'][y]['tblMBILInvoiceTaxCodes']) _temp_invoice['tblMBILInvoiceItems'][y]['tblMBILInvoiceTaxCodes'] = [];
